...
// Use the dedicated acquisition proxy; the monitor console owns /api on 8088.
function browserId() {
  try { if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") return crypto.randomUUID(); } catch {}
  return `tab-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
}
function acquisitionUrl(stationId:string) {
  const upstreamOrigin = `${location.protocol}//${location.hostname}:8081`;
  return `${upstreamOrigin}/_airbot_login?station_id=${encodeURIComponent(stationId)}`;
}
...
export default function App() {
  ...
  const [internetProgress, setInternetProgress] = useState<string|null>(null);
  ...
  async function enableInternetAccess() {
    setNetworkInfo(null);
    setInternetProgress("正在建立公网连接，可能需要 30–60 秒…");
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      try {
        ...
        setInternetProgress("公网连接暂时失败，正在自动重试（"+(attempt+2)+"/3）…");
  ...
  return <div className="app-shell">
    ...
    {internetProgress&&<div className="toast ok"><Loader2 className="spin"/>{internetProgress}</div>}
...
function TerminalWindow({station,onClose}:{station:Station;onClose:()=>void}) {
  ...
  const [tabs,setTabs]=useState([{id:browserId(),title:"终端 1"}]);
  const [active,setActive]=useState(tabs[0].id);
  const [maximized,setMaximized]=useState(false),[offset,setOffset]=useState({x:0,y:0});
  const windowDrag=useRef<{x:number;y:number;ox:number;oy:number}|null>(null);
  function addTab(){const id=browserId();setTabs(current=>[...current,{id,title:"终端 "+(current.length+1)}]);setActive(id)}
  ...
  function startDrag(event:React.PointerEvent<HTMLElement>){if(maximized||(event.target as HTMLElement).closest("button"))return;event.currentTarget.setPointerCapture(event.pointerId);windowDrag.current={x:event.clientX,y:event.clientY,ox:offset.x,oy:offset.y}}
  function moveWindow(event:React.PointerEvent<HTMLElement>){if(!windowDrag.current)return;setOffset({x:windowDrag.current.ox+event.clientX-windowDrag.current.x,y:windowDrag.current.oy+event.clientY-windowDrag.current.y})}
  ...
  return <div className="terminal-scrim"><section className={"terminal-window"+(maximized?" maximized":"")} style={maximized?undefined:{transform:"translate("+offset.x+"px,"+offset.y+"px)"}}><header className="terminal-titlebar" onPointerDown={startDrag} onPointerMove={moveWindow} onPointerUp={()=>windowDrag.current=null} onPointerCancel={()=>windowDrag.current=null}><div><SquareTerminal/><strong>{station.name}</strong><span>{station.ip}</span></div><button onClick={addTab}><Plus/>新建终端</button><button onClick={()=>{setMaximized(value=>!value);setOffset({x:0,y:0})}}>{maximized?"还原":"最大化"}</button><button onClick={onClose}><X/></button></header><nav>{tabs.map(tab=><button className={active===tab.id?"active":""} onClick={()=>setActive(tab.id)} key={tab.id}><span>{tab.title}</span><X onClick={e=>{e.stopPropagation();closeTab(tab.id)}}/></button>)}</nav><div className="terminal-panes">{tabs.map(tab=><TerminalPane key={tab.id} station={station} credentials={credentials} active={active===tab.id}/>)}</div></section></div>;
}
...
function TerminalPane({station,credentials,active}:{station:Station;credentials:TerminalCredentials;active:boolean}) {
  ...
  const [fileNotice,setFileNotice]=useState<{kind:"ok"|"error";text:string}|null>(null); const fileAction=useRef(""); const noticeTimer=useRef<number|undefined>(undefined);
  function notifyFile(kind:"ok"|"error",text:string){setFileNotice({kind,text});window.clearTimeout(noticeTimer.current);noticeTimer.current=window.setTimeout(()=>setFileNotice(null),4000)}
  ...
  function requestFile(file:RemoteFile,mode:"download"|"preview"){if(file.is_dir){setRemotePath(file.path);list(file.path);return}pendingMode.current=mode;fileAction.current=mode==="download"?"下载":"预览";pendingPath.current=file.path;setFileBusy(true);send({type:"file_download",path:file.path});}
  function receiveFile(name:string,data:string){try{const raw=atob(data),content=new Uint8Array(raw.length);for(let i=0;i<raw.length;i++)content[i]=raw.charCodeAt(i);const ext=name.split(".").pop()?.toLowerCase()||"";if(pendingMode.current==="preview"){if(["png","jpg","jpeg","gif","webp","svg"].includes(ext)){const type=ext==="svg"?"image/svg+xml":`image/${ext==="jpg"?"jpeg":ext}`;setPreview({name,path:pendingPath.current,url:URL.createObjectURL(new Blob([content],{type}))})}else if(["txt","log","json","yaml","yml","toml","ini","conf","cfg","xml","csv","md","py","sh","js","ts","tsx","css","html"].includes(ext)){setPreview({name,path:pendingPath.current,text:new TextDecoder().decode(content),editable:true});setSaveState("saved")}else setPreview({name,path:pendingPath.current,text:"该文件为二进制格式，请下载后查看。"});notifyFile("ok",name+" 预览成功");fileAction.current=""}else{const url=URL.createObjectURL(new Blob([content])),link=document.createElement("a");link.href=url;link.download=name;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);notifyFile("ok",name+" 下载成功")}setFileBusy(false)}catch(error){notifyFile("error",(fileAction.current||"预览")+"失败："+(error as Error).message);fileAction.current="";setFileBusy(false)}}
  function editPreview(text:string){if(!preview?.editable)return;setPreview({...preview,text});setSaveState("saving")} function savePreview(){if(!preview?.editable||saveState!=="saving")return;try{fileAction.current="编辑保存";const data=btoa(unescape(encodeURIComponent(preview.text||"")));send({type:"file_write",path:preview.path,data})}catch(error){setSaveState("error");notifyFile("error","编辑保存失败："+(error as Error).message)}} function upload(file?:File){if(!file)return;if(file.size>20*1024*1024){notifyFile("error","上传失败：单个文件不能超过 20 MB");return}fileAction.current="上传";setFileBusy(true);const reader=new FileReader();reader.onload=()=>send({type:"file_upload",path:remotePath,name:file.name,data:String(reader.result).split(",")[1]||""});reader.onerror=()=>{setFileBusy(false);notifyFile("error","上传失败：无法读取 "+file.name)};reader.readAsDataURL(file)} function removeFile(file:RemoteFile){if(!confirm("确认永久删除“"+file.name+"”吗？删除后无法恢复。"+(file.is_dir?" 目录必须为空。":"")))return;fileAction.current="删除";setFileBusy(true);send({type:"file_delete",path:file.path})}
  useEffect(()=>{
    ...
    const connect=()=>{if(disposed)return;terminal.writeln("\r\n\x1b[36m正在建立 SSH 连接…\x1b[0m");const socket=new WebSocket(`${location.protocol==="https:"?"wss":"ws"}://${location.host}/ws/terminal/${station.id}`);socket.binaryType="arraybuffer";socketRef.current=socket;socket.onopen=()=>socket.send(JSON.stringify({...credentials,cols:terminal.cols,rows:terminal.rows}));socket.onmessage=event=>{if(typeof event.data!=="string"){terminal.write(new Uint8Array(event.data));return}const message=JSON.parse(event.data);if(message.type==="error"){terminal.writeln("\r\n\x1b[31m"+message.message+"\x1b[0m");if(fileAction.current){notifyFile("error",fileAction.current+"失败："+message.message);if(fileAction.current==="编辑保存")setSaveState("error");fileAction.current=""}setFileBusy(false)}else if(message.type==="connected"){terminal.writeln("\r\n\x1b[32mSSH 已连接\x1b[0m");retryDelay=1000;setFileBusy(true);socket.send(JSON.stringify({type:"file_list",path:"."}))}else if(message.type==="file_list"){setRemotePath(message.path);setRemoteFiles(message.entries);setFileBusy(false)}else if(message.type==="file_uploaded"){notifyFile("ok",message.name+" 上传成功");fileAction.current="";socket.send(JSON.stringify({type:"file_list",path:message.path}))}else if(message.type==="file_download")receiveFile(message.name,message.data);else if(message.type==="file_written"){setSaveState("saved");fileAction.current="";notifyFile("ok","文件编辑保存成功")}else if(message.type==="file_deleted"){fileAction.current="";notifyFile("ok",message.name+" 删除成功");if(preview?.path&&preview.path.endsWith("/"+message.name))setPreview(null);socket.send(JSON.stringify({type:"file_list",path:message.path}))}};socket.onclose=()=>{if(disposed)return;terminal.writeln(`\r\n\x1b[33mSSH 已断开，${Math.round(retryDelay/1000)} 秒后自动重连…\x1b[0m`);retryTimer=window.setTimeout(connect,retryDelay);retryDelay=Math.min(retryDelay*2,15000)}};connect();
    ...
    return()=>{disposed=true;window.clearTimeout(retryTimer);window.clearTimeout(saveTimer.current);window.clearTimeout(noticeTimer.current);observer.disconnect();input.dispose();socketRef.current?.close();socketRef.current=null;terminal.dispose()};
  },[station.id,credentials]);
  useEffect(()=>{if(active)setTimeout(()=>fitRef.current?.fit(),20)},[active]);
  return <div className={"terminal-pane "+(active?"active":"")}>{fileNotice&&<div className={"toast "+fileNotice.kind}>{fileNotice.kind==="ok"?<CheckCircle2/>:<AlertTriangle/>}{fileNotice.text}</div>}<div className="terminal-layout"><div className="xterm-host" ref={container}/><aside className="remote-files"><header><div><Folder/><strong>远端文件</strong></div><button disabled={fileBusy} onClick={()=>uploadRef.current?.click()}><Upload/>上传</button><input ref={uploadRef} hidden type="file" onChange={e=>{upload(e.target.files?.[0]);e.target.value=""}}/></header><div className="remote-path"><button disabled={fileBusy} onClick={()=>{const path=parentPath(remotePath);setRemotePath(path);list(path)}}>↑</button><input value={remotePath} onChange={e=>setRemotePath(e.target.value)} onKeyDown={e=>e.key==="Enter"&&list(remotePath)}/><button disabled={fileBusy} onClick={()=>list(remotePath)}>{fileBusy?<Loader2 className="spin"/>:<RefreshCw/>}</button></div><div className="remote-list">{files.map(file=><div key={file.path} onDoubleClick={()=>requestFile(file,"preview")}><span onClick={()=>requestFile(file,"preview")}>{file.is_dir?<Folder/>:<FileText/>}<b>{file.name}</b><small>{file.is_dir?"目录":bytes(file.size)}</small></span>{!file.is_dir&&<button title="下载" onClick={()=>requestFile(file,"download")}><Download/></button>}<button className="delete" title="删除" onClick={()=>removeFile(file)}><Trash2/></button></div>)}{!files.length&&!fileBusy&&<p>目录为空</p>}</div></aside></div>{preview&&<div className="file-preview"><header><strong>{preview.name}</strong>{preview.editable&&<small className={"save-state "+saveState}>{saveState==="saving"?"有未保存修改":saveState==="error"?"保存失败":"已自动保存"}</small>}{preview.editable&&<button disabled={saveState!=="saving"} onClick={savePreview}>保存</button>}<button onClick={()=>{if(preview.url)URL.revokeObjectURL(preview.url);setPreview(null)}}><X/></button></header>{preview.url?<img src={preview.url}/>:preview.editable?<textarea value={preview.text||""} onChange={e=>editPreview(e.target.value)} spellCheck={false}/>:<pre>{preview.text}</pre>}</div>}</div>;
}
